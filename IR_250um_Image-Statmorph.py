...
def multiprocessing_func(x):
    ident=x[0]
    size1=int(x[3])
    image1=(fits.getdata(x[1], ext=0)[71])
    ...
    try:
    	...
    	for numobj,objs in enumerate(source_morphs) :
        	if abs(math.dist((sby2-0.5,sby2-0.5),(objs.xc_asymmetry,objs.yc_asymmetry)))<mindist :
            		mindist=abs(math.dist((),(objs.xc_asymmetry,objs.yc_asymmetry)))
            		maxnum=numobj
...
if __name__ == '__main__':
    ...
    pool.close()
    df.to_csv("/home/projects/dtu_00026/people/paupun/dust_continuum/output/IRStatMorphResults250um.csv")
