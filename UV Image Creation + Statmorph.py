...
def multiprocessing_func(x):
    ...
    try:
        ...
        print(maxnum)
        rarr=[ident,str(morph.xc_asymmetry),str(morph.yc_asymmetry),str((morph.rhalf_circ)*(60/size1)),str((morph.rhalf_ellip)*(60/size1)),str(morph.flag),str(objcount),str(morph.concentration),str(morph.asymmetry),str(morph.smoothness)]
    except:
        pass
    return rarr
...
if __name__ == '__main__':
    ...
    df=pd.DataFrame({'ID':ids,'Center x':cenx,'Center y':ceny,'HLR circ':rcirc,"HLR ellip":rellip,"Flag":flag, "Total objects":numobjs,"C":Cas,"A":cAs,"S":caS})
    print(df)
    df.to_csv("/home/projects/dtu_00026/people/paupun/dust_continuum/output/NewResults.csv")
