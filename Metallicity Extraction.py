    ...
    for y in regions :
        ...
        for z in galaxies:
            ...
            with h5py.File(fname, 'r') as hf:
                print(list(hf[regionnumber+'/'+redshiftcode+'/Galaxy/Metallicity'].keys()))
                #print(list(hf[regionnumber+'/'+redshiftcode+'/Particle'].keys()))
                BH_Mass = np.array(hf[regionnumber+'/'+redshiftcode+'/Galaxy/Metallicity/MassWeightedGasZ'], dtype = np.float64)
                bm.append(BH_Mass[ind])
            t+=1
            print("Galaxies Done :",t,BH_Mass[ind])
df=pd.DataFrame({'ID':ids,'Metallicity':bm})
print(df)
df.to_csv("/home/projects/dtu_00026/people/paupun/dust_continuum/output/Metallicity.csv")
